...
class DictKeysDict(dict):
    ...
    def _ensure_frozendict(self, key):
        ...
        if isinstance(key, str):
            ...
            raise KeyError("Keys must be instances of FrozenDict or convertible strings")
        return key
    ...
    def _convert_value(self, value):
        ...
        if isinstance(value, dict) and not isinstance(value, DictKeysDict):
            return DictKeysDict({self._ensure_frozendict(k): self._convert_value(v) for k, v in value.items()})
    ...
    def loadJson(cls, jsonStr):
        ...
        def convert(obj):
            if isinstance(obj, dict):
                return DictKeysDict({FrozenDict.fromString(k): convert(v) for k, v in obj.items()})
